...
C<Text::Parser> adopts the principle that errors should be indicated using exceptions, and not a special return code or error code. This class throws a number of different types of exceptions that are described in L<Text::Parser::Errors>. The base class for all these exceptions is C<Text::Parser::Errors::GenericError>. In addition to this, wrong use of any of the methods can cause exceptions of C<L<Moose::Exception>> to be thrown.  L<This example|/"Example 2 : Error checking"> shows how you could create your own exceptions. You can optionally build a handler using L<Dispatch::Class>, to handle the various types of exceptions.
