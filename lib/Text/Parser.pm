...
The above code reads the first command-line argument as a string, and assuming it is the name of a text file, it will print the content of the file to C<STDOUT>. If the string is not the name of a text file it will throw an exception and exit.
    ...
    # Rule conditions will be tested and corresponding rules executed in the order they are defined.
    # One of 'if' or 'do' must be supplied. 'name' is optional.
    # Notice awk-style variables in the syntax for 'if' and 'do'.
    #   You could use $1, $2, $3 etc., to get the fields in order like in AWK.
    #   You could also use ${-1}, ${-2}, etc., to access fields from the last.
    #   $0 is the whole line. $_ is also set as the whole line, to ease regex matches.
    #   $this is a special variable which points to the $parser object.
    #
    ...
    $parser->awk_like_rule(
        name => 'read_name',            # Optional, auto-generated when not specified
        if   => '$1 eq "NAME:"',        # If 'do' is specified, this conditional is optional
                                        # If 'if' is not specified, the rule is always executed
        do   => 'my (@fld) = $this->field_range(1, -1);
                 return "@fld";',       # If 'if' is specified, then 'do' parameter is optional
                                        # Default 'do' is same as 'return $0;'
                                        # One of 'if' or 'do' must be specified.
    );
...
The above code extracts the name from lines where the first word is C<NAME:>. Below, we'll parse the names and other fields using a parser class derived from C<Text::Parser>.
    ...
    # Syntax similar to awk_like_rule method of Text::Parser class
    # One of 'if' or 'do' must be supplied. 'name' is optional.
    # Rule conditions will be tested and corresponding rules executed in the order they are defined.
    # Note that the rules defined in a class will be compiled and checked during compile-time, 
    # whereas in earlier example they were compiled and checked during runtime.
    ...
    awk_style_rule if => '$1 eq "EMAIL:"', 
        do => 'my $last = $this->pop_record;
               $last->{email} = $2;
               return $last;';

    # If 'do' is specified, then 'if' is an optional conditional. If no
    # conditional is specified, the rule will be executed always
    awk_style_rule do => 'return $this->pop_record;';

    # If 'if' conditional is specified, and 'do' is not specified, then
    # the implied meaning is that the whole line is returned as a record
    # Essentially, the default value for 'do' is 'return $0;'
    awk_style_rule if => '$1 eq "THIS WILL NEVER MATCH"';

    sub BUILD {
        my $self = shift;
        $self->rules_source(__PACKAGE__);
            # Automatically sets:
            #   auto_split => 1, 
            #   auto_chomp => 1, 
            #   auto_trim  => 'b', 
