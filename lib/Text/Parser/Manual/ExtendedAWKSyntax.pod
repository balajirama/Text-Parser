...
The idea of storing data in the C<$this> variable is so cool that I had to make it easier to use. So you get what are called I<"stashed variable">s. Internally these variables are just stored in a hash, but it is as if you are stashing away useful data. You can store any scalar, hashref or arrayref. Notice that all of them must be references.
...
* In AWK if you use a positional identifier like C<$8> when there are only 7 fields on a line, it evaluates to empty string. In ExAWK, if you use C<$8> in either the condition or task strings, an automatic pre-condition is generated to check that there must be at least 8 fields on the input line.

L<Table of contents|Text::Parser::Manual> | L<Previous|Text::Parser::Manual::ComparingWithNativePerl> | L<Next|Text::Parser::Manual::CollectedExamples>
